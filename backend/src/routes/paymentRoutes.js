...
router.post('/', async (req, res) => {
  try {
    const { user, seats, status, event, funcion, referralCode } = req.body;
    ...
    const seatTotal = seats.reduce((sum, seat) => sum + (seat.price || 0), 0);
    const commissionRate = parseFloat(process.env.REFERRAL_RATE || '0.05');
    const referralCommission = referrer && status === 'pagado' ? seatTotal * commissionRate : 0;
...
// Payments generated by referrals for a user
router.get('/referrer/:referrerId', async (req, res) => {
  try {
    const payments = await Payment.find({ referrer: req.params.referrerId, status: 'pagado' });
    const totalCommission = payments.reduce((sum, p) => sum + (p.referralCommission || 0), 0);
