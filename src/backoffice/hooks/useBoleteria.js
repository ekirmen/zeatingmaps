...
export const useBoleteria = () => {
      ...
      try {
        ...
        if (savedCart) {
          ...
          if (parsedCart.length > 0 && parsedCart[0]?.funcionId === selectedFuncion.id) {
            ...
            logger.log(
              'ðŸ”„ [useBoleteria] Carrito restaurado desde localStorage:',
              parsedCart.length,
              'items'
            );
          }
  ...
  const saveCarritoToStorage = useCallback(newCarrito => {
    try {
      ...
      logger.log(
        'ðŸ’¾ [useBoleteria] Carrito guardado en localStorage:',
        newCarrito.length,
        'items'
      );
    } catch (error) {
  ...
  const setCarritoMemo = useCallback(
    updater => {
  ...
  const setSelectedEventMemo = useCallback(
    newEvent => {
  ...
  const removeFromCarrito = useCallback(
    asientoId => {
      setCarritoMemo(prev => {
        const safePrev = Array.isArray(prev) ? prev : [];
        return safePrev.filter(
          item => item.id !== asientoId && item._id !== asientoId && item.sillaId !== asientoId
        );
      });
  ...
  const handleFunctionSelect = useCallback(
    async (functionId, options = {}) => {
      logger.log(
        'ðŸ”„ [useBoleteria] handleFunctionSelect called with function ID:',
        functionId,
        'options:',
        options
      );
      ...
      try {
        ...
        const mappedSala =
          typeof salaField === 'object' && salaField !== null
            ? salaField
            : salaField
              ? { id: salaField }
              : null;
        ...
        if (funcionData.plantilla) {
          ...
          if (funcionData.plantilla_entradas) {
            try {
              ...
              if (plantillaError) {
                logger.error(
                  'Œ Error cargando plantilla desde plantilla_entradas:',
                  plantillaError
                );
        ...
        const salaId =
          mappedSala?.id || mappedSala?._id || salaField || funcionData.sala_id || null;
        ...
        if (salaId) {
          ...
          try {
            logger.log(
              'ðŸ” [useBoleteria] Llamando a fetchMapa con salaId:',
              salaId,
              'y funcionId:',
              funcionData.id
            );
            ...
            if (mapData && mapData.contenido) {
              ...
              if (Array.isArray(elementos)) {
                ...
                logger.warn(
                  'š ï¸ [useBoleteria] Mapa cargado pero sin contenido v¡lido o no es array',
                  {
                    mapData: mapData,
              ...
              if (
                totalSeats === 0 &&
                mapData.contenido.zonas &&
                Array.isArray(mapData.contenido.zonas)
              ) {
  ...
  const handleEventSelect = useCallback(
    async eventoId => {
      ...
      try {
        ...
        const funcionesMapeadas = (funcionesData || [])
          .filter(funcion => funcion && funcion.id)
          .map(funcion => ({
            ...funcion,
            sala:
              typeof funcion.sala === 'object' && funcion.sala !== null
                ? funcion.sala
                : funcion.sala
                  ? { id: funcion.sala }
                  : null,
    ...
    const fetchEventos = async () => {
      ...
      try {
        ...
        if (storedEventId && data && Array.isArray(data)) {
          ...
          logger.log(
            'ðŸ”„ [useBoleteria] No hay evento guardado, seleccionando el primero disponible'
          );
