...
const AuditSystem = () => {
  ...
  const columns = [
      ...
      render: tenant =>
        tenant ? (
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#666' }}>{tenant.subdomain}.ticketera.com</div>
          </div>
        ) : (
    ...
    },
    {
      ...
      key: 'actions',
      render: (_, record) => (
        <Button size="small" icon={<EyeOutlined />} onClick={() => handleViewLog(record)}>
          Ver
        </Button>
      ),
    },
        ...
        {selectedLog && (
          <div>
            ...
            <Descriptions title="Información del Log" bordered column={1}>
              <Descriptions.Item label="ID del Log">{selectedLog.id}</Descriptions.Item>
