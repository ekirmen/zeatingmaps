...
const EventForm = ({ eventData = null, onSave, onCancel, loading = false }) => {
  ...
  useEffect(() => {
    if (eventData) {
      ...
      if (eventData.imagenes) {
        try {
          const images =
            typeof eventData.imagenes === 'string'
  ...
  const handleImageRemove = async imageType => {
    try {
      ...
      if (imageData?.bucket && imageData?.url) {
        const { error } = await supabase.storage.from(imageData.bucket).remove([imageData.url]);
    ...
    <Form.Item label={label}>
      <div style={{ marginBottom: 16 }}>
        ...
        <Space>
          ...
          {previewImages[imageType] && (
            <Button danger icon={<DeleteOutlined />} onClick={() => handleImageRemove(imageType)}>
              Eliminar
            </Button>
          )}
        </Space>
  ...
  return (
    <Card>
      ...
      >
        <Title level={4}>{eventData ? 'Editar Evento' : 'Crear Nuevo Evento'}</Title>
        ...
        >
          <TextArea rows={4} placeholder="Describe el evento..." />
        </Form.Item>
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item name="precio" label="Precio Base">
              <InputNumber
                min={0}
                step={0.01}
                style={{ width: '100%' }}
        ...
        <Form.Item name="activo" label="Evento Activo" valuePropName="checked">
          <Switch />
        </Form.Item>
        ...
        <Row gutter={16}>
          <Col span={8}>
            <ImageUploader imageType="banner" label="Banner Principal" required />
          </Col>
          <Col span={8}>
            <ImageUploader imageType="portada" label="Imagen de Portada" />
          </Col>
          <Col span={8}>
            <ImageUploader imageType="obraImagen" label="Imagen de Obra" />
          </Col>
        </Row>

        <Row gutter={16}>
          <Col span={8}>
            <ImageUploader imageType="logoHorizontal" label="Logo Horizontal" />
          </Col>
          <Col span={8}>
            <ImageUploader imageType="logoVertical" label="Logo Vertical" />
          </Col>
          <Col span={8}>
            <ImageUploader imageType="bannerPublicidad" label="Banner Publicidad" />
          </Col>
        </Row>
