  ...
  const validationRules = [
    {
      ...
      validate: mapa => {
        ...
        if (
          !mapa.contenido?.configuracion?.dimensions?.width ||
...

    {
      ...
      validate: mapa => {
        ...
        elementos.forEach((el, index) => {
          if (
            !el.posicion ||
            typeof el.posicion.x !== 'number' ||
            typeof el.posicion.y !== 'number'
          ) {
    ...
    <div className="max-w-6xl mx-auto">
      <Card>
        ...
        <Paragraph className="text-gray-600 mb-6">
          Antes de publicar el mapa, es importante verificar que todo est© correctamente
          configurado. La validaci³n revisar¡ la integridad, estructura y configuraci³n del mapa.
        </Paragraph>
                ...
                {results.isValid ? (
                  <div>
                    ...
                    <Text className="text-gray-600">
                      Tu mapa ha pasado todas las verificaciones cr­ticas y est¡ listo para
                      continuar.
                    </Text>
                    <div className="mt-4">
                      <Button type="primary" size="large" icon={<CheckOutlined />} onClick={onNext}>
                        Continuar
                      </Button>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Text className="text-gray-600">
                      Hay errores que deben corregirse antes de continuar. Revisa la lista de
                      errores arriba.
                    </Text>
                  </div>
                )}
          ...
          <Collapse defaultActiveKey={['0']}>
            {validationRules.map((rule, index) => (
              <Panel
                ...
                key={index}
              >
                <Text className="text-gray-600 mb-2 block">{rule.description}</Text>

                {results && results.errors && (
                  <div className="mt-2">
                    {results.errors.some(
                      error => error.includes(rule.name.toLowerCase()) || error.includes(rule.id)
                    ) ? (
