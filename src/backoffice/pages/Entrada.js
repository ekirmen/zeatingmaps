...
const tiposDeProducto = [
  ...
  {
    label: 'Invitaciones o cortesías',
    description: 'Entradas de cortesía.',
    value: 'Invitaciones',
  },
  ...
  {
    label: 'Promoción (peores asientos)',
    description: 'Asientos con visibilidad reducida.',
    value: 'Promocion',
  },
  {
    label: 'Pase de temporada',
    description: 'Incluido en un abono de temporada.',
    value: 'PaseTemporada',
  },
...
const Entrada = () => {
  ...
  const handleSaveData = async datos => {
    ...
    const { error } = await supabase.from('entradas').insert([datosAInsertar]); // insert espera un array
  ...
  const handleSaveEditData = async datos => {
    ...
    const { error } = await supabase.from('entradas').update(datosAInsertar).eq('id', ticketId);
  ...
  const handleDeleteTicket = async ticketId => {
    if (window.confirm('¿Estás seguro de que quieres eliminar esta entrada?')) {
      const { error } = await supabase.from('entradas').delete().eq('id', ticketId);
            ...
            <div className="px-4 py-3 border-b border-gray-200">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-xs text-gray-600">
                  {tickets.length} entrada{tickets.length !== 1 ? 's' : ''} configurada
                  {tickets.length !== 1 ? 's' : ''}
                </div>
              </div>
            </div>
              ...
              <div className="p-3">
                <div className="space-y-2">
                  {tickets.map(t => (
                    <div
                      key={t._id}
                      className="flex flex-wrap items-center justify-between gap-3 border rounded px-3 py-2 bg-white"
                    >
                      <div className="font-medium">
                        {t.nombreEntrada || t.nombre || t.producto || 'Entrada'}
                      </div>
                      <div className="text-sm text-gray-600">
                        Tipo: {t.tipo || t.tipo_producto || 'General'}
                      </div>
                      <div className="text-sm text-gray-600">
                        Cantidad: {t.min || 1} - {t.max || 10}
                      </div>
                      <div className="text-sm text-gray-600">
                        IVA: {t.iva_info?.porcentaje ?? t.iva ?? 0}
                      </div>
