  ...
  const loadTags = async () => {
    try {
      ...
      if (userError) {
        ...
        const processedUserTags = await Promise.all(
          (userData || []).map(async tag => {
            try {
              ...
              const userIds = (relations || []).map(relation => relation.user_id).filter(Boolean);
              ...
              if (userIds.length > 0) {
                ...
                if (!profilesError && profilesData) {
                  ...
                  users = userIds.map(id => profilesMap.get(id)).filter(Boolean);
                }
              }
        ...
        tags.map(tag => (
          <Card key={tag.id} size="small">
            <div className="flex items-center justify-between">
              ...
              <Space>
                <Button type="link" icon={<EditOutlined />} onClick={() => handleEdit(tag)}>
                  Editar
                </Button>
                <Button
                  type="link"
          ...
          {activeTab === 'user' && (
            <>
              <Form.Item name="description" label="Descripci³n">
                <Input.TextArea rows={3} placeholder="Descripci³n opcional del tag" />
              </Form.Item>
