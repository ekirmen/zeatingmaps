...
const AuthStatus = ({ showDetails = false }) => {
  ...
  if (showDetails) {
    return (
      ...
      >
        ...
        <p>
          <strong>Autenticado:</strong> {authState.isAuthenticated ? '✅ Sí' : '❌ No'}
        </p>
        {authState.user && (
          <>
            <p>
              <strong>Usuario:</strong> {authState.user.email}
            </p>
            <p>
        ...
        {authState.session && (
          <p>
            <strong>Token:</strong> {authState.session.access_token ? '✅ Presente' : '❌ Ausente'}
          </p>
        )}
      </div>
    );
  ...
  return (
    ...
    >
      {authState.isAuthenticated ? `✅ ${authState.user?.email}` : '❌ No autenticado'}
    </div>
  );
};
