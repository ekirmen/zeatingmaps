    ...
    <div className="p-4 space-y-4">
      ...
      {selectedEvent && functions.length > 0 && (
        <List
          ...
          header={
            <div>
              Funciones para: <strong>{selectedEvent.nombre}</strong>
            </div>
          }
      ...
      {mapa && selectedFunc && (
        <div className="border p-4">
          <h3>
            Mapa de asientos para la funci³n del{' '}
            {selectedFunc.fechaCelebracion
              ? new Date(selectedFunc.fechaCelebracion).toLocaleString()
              : 'fecha no disponible'}
          </h3>
          <LoadOnVisible
