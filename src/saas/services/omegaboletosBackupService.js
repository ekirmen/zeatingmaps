...
class OmegaboletosBackupService {
  constructor() {
    this.backupConfig = {
      // Configuración específica para omegaboletos.com
      omegaboletos: {
        domain: 'omegaboletos.com',
        base_path: '/home/omegaboletos/public_html/omegaboletos.com',
        database_path: '/home/omegaboletos/public_html/omegaboletos.com/database',
        files_path: '/home/omegaboletos/public_html/omegaboletos.com/files',
        configs_path: '/home/omegaboletos/public_html/omegaboletos.com/configs',
        temp_path: '/home/omegaboletos/public_html/omegaboletos.com/temp'
      },
  ...
  // Crear backup de base de datos desde Supabase hacia omegaboletos.com
  async createDatabaseBackup(type = 'daily') {
    try {
      const timestamp = new Date().toISOString().split('T')[0];
      const backupFileName = `backup_${type}_${timestamp}.sql`;
      const backupDir = path.join(this.backupConfig.omegaboletos.database_path, type);
      ...
      await this.logBackupAction('database_backup_created', {
        ...
        source: 'supabase',
        destination: 'omegaboletos.com_and_github'
      });
  ...
  // Crear backup de archivos de tenants desde Supabase Storage hacia omegaboletos.com
  async createFilesBackup(type = 'daily') {
    try {
      const timestamp = new Date().toISOString().split('T')[0];
      const backupFileName = `files_${type}_${timestamp}.tar.gz`;
      const backupDir = path.join(this.backupConfig.omegaboletos.files_path, type);
      const backupPath = path.join(backupDir, backupFileName);
      ...
      const tempDir = this.backupConfig.omegaboletos.temp_path;
      ...
      for (const tenant of tenants) {
        ...
        if (tenantFiles.length > 0) {
          ...
          // Guardar archivos en omegaboletos.com
  ...
  async createConfigBackup() {
    try {
      ...
      const backupPath = path.join(this.backupConfig.omegaboletos.configs_path, backupFileName);

      // Crear directorio si no existe
      await this.ensureDirectoryExists(this.backupConfig.omegaboletos.configs_path);
      ...
      const configs = {
        ...
        omegaboletos_config: this.backupConfig.omegaboletos,
        backup_metadata: {
          created_at: new Date().toISOString(),
          version: process.env.APP_VERSION || '1.0.0',
          environment: process.env.NODE_ENV || 'production',
          source: 'github_repository',
          backup_destination: 'omegaboletos.com'
        }
      };

      // Guardar configuraciones en omegaboletos.com
  ...
  async restoreFromBackup(backupType, backupFile, targetTenant = null) {
    try {
      ...
      if (backupType === 'database') {
        backupPath = path.join(this.backupConfig.omegaboletos.database_path, 'daily', `${backupFile}.gz`);
        await this.restoreDatabase(backupPath);
      } else if (backupType === 'files') {
        backupPath = path.join(this.backupConfig.omegaboletos.files_path, 'daily', backupFile);
        await this.restoreFiles(backupPath, targetTenant);
      } else if (backupType === 'config') {
        backupPath = path.join(this.backupConfig.omegaboletos.configs_path, backupFile);
  ...
  async verifyBackupIntegrity(backupFile) {
    try {
      const backupPath = path.join(this.backupConfig.omegaboletos.database_path, backupFile);
...
const omegaboletosBackupServiceInstance = new OmegaboletosBackupService();
export default omegaboletosBackupServiceInstance;
