-- ==========================================
-- 0. Asegurar Columnas en seat_locks
-- ==========================================
DO $$
BEGIN
...
BEGIN
    ...
    AND (seat_locks.expires_at > NOW() OR seat_locks.status IN ('vendido', 'pagado', 'reservado', 'bloqueado'))
    FOR UPDATE;

    IF v_existing_lock.id IS NOT NULL THEN
        -- Extender si es la misma sesión
        IF v_existing_lock.session_id::text = p_session_id AND v_existing_lock.status NOT IN ('vendido', 'pagado', 'reservado', 'bloqueado') THEN
             ...
             SET expires_at = v_expires_at,
                 locked_at = NOW(),
                 tenant_id = COALESCE(p_tenant_id::uuid, seat_locks.tenant_id),
    ...
    ON CONFLICT (seat_id, funcion_id) 
    DO UPDATE SET 
...
-- Alias para compatibilidad con versiones anteriores si es necesario
...
BEGIN
    ...
    AND seat_locks.funcion_id = p_funcion_id
    AND (
        seat_locks.expires_at > NOW() 
        OR seat_locks.status IN ('vendido', 'pagado', 'reservado', 'bloqueado')
    )
    AND (p_session_id IS NULL OR seat_locks.session_id != p_session_id::uuid); 
