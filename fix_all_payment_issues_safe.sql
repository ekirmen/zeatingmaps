-- =====================================================
-- SOLUCIONAR TODOS LOS PROBLEMAS DE PAGO (VERSIÓN SEGURA)
-- =====================================================
...
-- 2. CREAR TABLA NOTIFICATIONS COMPLETA (SI NO EXISTE)
...
-- 3. AGREGAR COLUMNA 'read' SI NO EXISTE
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    ELSE
        RAISE NOTICE 'Columna read ya existe en notifications';
    END IF;
END $$;

-- 4. CREAR ÍNDICES PARA NOTIFICATIONS (SI NO EXISTEN)
...
-- 5. CREAR TRIGGER PARA NOTIFICATIONS (SI NO EXISTE)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Trigger update_notifications_updated_at creado';
    ELSE
        RAISE NOTICE 'Trigger update_notifications_updated_at ya existe';
    END IF;
END $$;
...
SELECT 'notifications read column exists' as check_type,
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'read' AND table_schema = 'public')
            THEN 1 ELSE 0 END as column_count
