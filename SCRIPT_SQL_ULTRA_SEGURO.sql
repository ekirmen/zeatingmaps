-- =====================================================
-- SCRIPT SQL ULTRA SEGURO - OPTIMIZACIÓN DE BASE DE DATOS
-- =====================================================

-- 1. VERIFICAR ESTRUCTURA DE TABLA PAYMENTS
-- =====================================================

-- Crear una función temporal para verificar columnas
...
-- Verificar qué columnas existen en la tabla payments
DO $$
DECLARE
    ...
    has_payment_gateway_id boolean;
...
BEGIN
    ...
    SELECT column_exists('payments', 'updated_at') INTO has_updated_at;
    SELECT column_exists('payments', 'created_at') INTO has_created_at;
    SELECT column_exists('payments', 'seats') INTO has_seats;
    SELECT column_exists('payments', 'monto') INTO has_monto;
    SELECT column_exists('payments', 'usuario_id') INTO has_usuario_id;
    SELECT column_exists('payments', 'event') INTO has_event;
    SELECT column_exists('payments', 'funcion') INTO has_funcion;
    SELECT column_exists('payments', 'processed_by') INTO has_processed_by;
    SELECT column_exists('payments', 'payment_gateway_id') INTO has_payment_gateway_id;
    SELECT column_exists('payments', 'tenant_id') INTO has_tenant_id;
    SELECT column_exists('payments', 'locator') INTO has_tenant_id;
    SELECT column_exists('payments', 'status') INTO has_status;
    SELECT column_exists('payments', 'id') INTO has_id;
    
    RAISE NOTICE '=== ESTRUCTURA DE TABLA PAYMENTS ===';
    RAISE NOTICE 'id: %', has_id;
    RAISE NOTICE 'created_at: %', has_created_at;
    RAISE NOTICE 'updated_at: %', has_updated_at;
    RAISE NOTICE 'seats: %', has_seats;
    RAISE NOTICE 'monto: %', has_monto;
    RAISE NOTICE 'usuario_id: %', has_usuario_id;
    RAISE NOTICE 'event: %', has_event;
    RAISE NOTICE 'funcion: %', has_funcion;
    RAISE NOTICE 'processed_by: %', has_processed_by;
    RAISE NOTICE 'payment_gateway_id: %', has_payment_gateway_id;
    RAISE NOTICE 'tenant_id: %', has_tenant_id;
    RAISE NOTICE 'locator: %', has_tenant_id;
    RAISE NOTICE 'status: %', has_status;
END $$;

-- 2. AGREGAR COLUMNAS FALTANTES A PAYMENT_TRANSACTIONS
-- =====================================================
...
-- 3. MIGRACIÓN SEGURA DESDE PAYMENTS
-- =====================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payments') THEN
        ...
        -- Migrar datos con columnas disponibles
        IF has_updated_at AND has_created_at THEN
            ...
            RAISE NOTICE 'Migración completada con created_at y updated_at';
            
        ELSIF has_created_at THEN
            -- Caso 2: Solo created_at existe
            EXECUTE '
                INSERT INTO payment_transactions (
                    id, order_id, amount, currency, status, created_at, updated_at,
                    user_id, evento_id, tenant_id, locator, funcion_id, payment_method,
            ...
            RAISE NOTICE 'Migración completada usando created_at para updated_at';
            
        ELSE
            -- Caso 3: Ninguna columna de fecha existe
            EXECUTE '
                INSERT INTO payment_transactions (
                    id, order_id, amount, currency, status, created_at, updated_at,
                    user_id, evento_id, tenant_id, locator, funcion_id, payment_method,
            ...
            RAISE NOTICE 'Migración completada usando now() para fechas';
        END IF;
...
-- 7. CREAR TABLA CONSOLIDADA DE MÉTODOS DE PAGO
-- =====================================================
...
-- 8. MIGRAR MÉTODOS DE PAGO (Solo si existen las tablas)
-- =====================================================
