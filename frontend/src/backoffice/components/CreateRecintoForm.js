...
const geocodeAddress = async (address) => {
  if (process.env.REACT_APP_GOOGLE_MAPS_API_KEY) {
    const googleRes = await fetch(
      `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(address)}&region=ve&key=${process.env.REACT_APP_GOOGLE_MAPS_API_KEY}`
    );
  ...
  const res = await fetch(
    `https://nominatim.openstreetmap.org/search?format=json&countrycodes=ve&q=${encodeURIComponent(address)}`
  );
