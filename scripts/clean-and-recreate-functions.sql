-- Script para limpiar y recrear las funciones RPC
-- Ejecutar en Supabase SQL Editor

-- 1. Eliminar todas las versiones existentes de las funciones
DROP FUNCTION IF EXISTS atomic_seat_lock(text, integer, uuid, text) CASCADE;
DROP FUNCTION IF EXISTS atomic_seat_lock(unknown, integer, uuid, unknown) CASCADE;
DROP FUNCTION IF EXISTS atomic_seat_unlock(text, integer, uuid) CASCADE;
DROP FUNCTION IF EXISTS is_seat_available(text, integer) CASCADE;

-- 2. Crear la función para bloquear asiento atómicamente
CREATE OR REPLACE FUNCTION atomic_seat_lock(
    p_seat_id text,
...
-- 3. Crear la función para desbloquear asiento
CREATE OR REPLACE FUNCTION atomic_seat_unlock(
    p_seat_id text,
    p_funcion_id integer,
    p_session_id uuid
)
...
-- 4. Crear la función para verificar disponibilidad
...
-- 5. Otorgar permisos a los roles
