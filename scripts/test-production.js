// Script para testear endpoints en producción
...
async function testEndpoint(name, method, url, body = null) {
  try {
    ...
    if (response.ok) {
      console.log(`${colors.green}✅ ${name}${colors.reset} - Status: ${response.status} - Time: ${responseTime}ms`);
      return { success: true, data, responseTime };
    } else {
      console.log(`${colors.red}❌ ${name}${colors.reset} - Status: ${response.status} - Time: ${responseTime}ms - Error: ${data.message || 'Unknown error'}`);
      return { success: false, error: data.message, responseTime };
    }
  } catch (error) {
...
// Función para testear endpoints principales
async function testProductionEndpoints() {
  console.log(`${colors.bold}${colors.blue}🚀 Testeando Endpoints de Producción${colors.reset}`);
  ...
  results.totalTime += gridLoad.responseTime || 0;
  ...
  results.totalTime += gridValidate.responseTime || 0;
  ...
  results.totalTime += eventsList.responseTime || 0;
  ...
  results.totalTime += eventBySlug.responseTime || 0;
  ...
  results.totalTime += dashboardStats.responseTime || 0;
  ...
  results.totalTime += userManagement.responseTime || 0;
  ...
  results.totalTime += salesReport.responseTime || 0;
  ...
  results.totalTime += testStripe.responseTime || 0;
  ...
  results.totalTime += testPayPal.responseTime || 0;
  ...
  results.totalTime += functionsList.responseTime || 0;
  ...
  results.totalTime += zonesList.responseTime || 0;
  ...
  results.totalTime += salesList.responseTime || 0;
  ...
  results.totalTime += clientsList.responseTime || 0;
  ...
  results.totalTime += clientsSearch.responseTime || 0;
  ...
  results.totalTime += venuesList.responseTime || 0;

  // Resumen final
  console.log(`\n${colors.bold}${colors.blue}📊 RESUMEN DE TESTS DE PRODUCCIÓN${colors.reset}`);
  console.log('═'.repeat(60));
  console.log(`${colors.bold}🌐 URL Base: ${API_BASE_URL}${colors.reset}`);
  ...
  console.log(`${colors.bold}⏱️  Tiempo total: ${results.totalTime}ms${colors.reset}`);
  console.log(`${colors.bold}⏱️  Tiempo promedio: ${Math.round(results.totalTime / results.total)}ms${colors.reset}`);
  ...
  if (results.failed > 0) {
    ...
    console.log('• Problemas de CORS o autenticación');
  }

  console.log(`\n${colors.bold}${colors.blue}🎉 Test de producción completado!${colors.reset}`);
  console.log(`\n${colors.bold}🔗 URLs para probar manualmente:${colors.reset}`);
...
module.exports = { testProductionEndpoints, testEndpoint };
