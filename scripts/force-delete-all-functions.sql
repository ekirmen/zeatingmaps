-- Script para eliminar TODAS las funciones de una vez
-- Ejecutar en Supabase SQL Editor

-- 1. Usar un bucle para eliminar todas las funciones relacionadas
DO $$
DECLARE
    func_record record;
BEGIN
    ...
    LOOP
        ...
        RAISE NOTICE 'Eliminada función: %', func_record.specific_name;
    END LOOP;
    ...
    -- Eliminar todas las versiones de seat_lock_atomic
    FOR func_record IN 
        SELECT routine_name, specific_name
        FROM information_schema.routines 
        WHERE routine_name = 'seat_lock_atomic' 
        AND routine_schema = 'public'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.specific_name || ' CASCADE';
        RAISE NOTICE 'Eliminada función: %', func_record.specific_name;
    ...
    -- Eliminar todas las versiones de seat_unlock_atomic
    FOR func_record IN 
        SELECT routine_name, specific_name
        FROM information_schema.routines 
        WHERE routine_name = 'seat_unlock_atomic' 
        AND routine_schema = 'public'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.specific_name || ' CASCADE';
        RAISE NOTICE 'Eliminada función: %', func_record.specific_name;
    ...
    -- Eliminar todas las versiones de seat_check_available
    FOR func_record IN 
        SELECT routine_name, specific_name
        FROM information_schema.routines 
        WHERE routine_name = 'seat_check_available' 
        AND routine_schema = 'public'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.specific_name || ' CASCADE';
        RAISE NOTICE 'Eliminada función: %', func_record.specific_name;
