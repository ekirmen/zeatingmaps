-- Script de debug SIN JOINs para evitar errores de columnas
-- Ejecutar este script para diagnosticar el problema de forma segura
...
-- 10. Mostrar tenants disponibles (sin JOIN, usando *)
SELECT 
...
-- 11. Verificar si hay eventos con tenant_id válido
SELECT 
  'Verificación de Relación Eventos-Tenants' as test,
  COUNT(*) as total_eventos,
  COUNT(CASE WHEN tenant_id IS NOT NULL THEN 1 END) as eventos_con_tenant,
  COUNT(CASE WHEN tenant_id IS NULL THEN 1 END) as eventos_sin_tenant,
  COUNT(CASE WHEN tenant_id IN (SELECT id FROM tenants) THEN 1 END) as eventos_con_tenant_valido
FROM eventos;
