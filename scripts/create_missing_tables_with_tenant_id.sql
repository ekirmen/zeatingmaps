-- Script para crear las tablas faltantes con tenant_id
...
-- 13. Insertar datos de ejemplo para testing
INSERT INTO system_alerts (tenant_id, title, message, type, priority, status) VALUES
    ((SELECT id FROM tenants LIMIT 1), 'Sistema Operativo', 'El sistema está funcionando correctamente', 'success', 'low', 'active'),
    ((SELECT id FROM tenants LIMIT 1), 'Mantenimiento Programado', 'Mantenimiento programado para mañana', 'warning', 'medium', 'active');

INSERT INTO admin_notifications (tenant_id, title, message, type, priority) VALUES
    ((SELECT id FROM tenants LIMIT 1), 'Bienvenido al Sistema', 'Tu cuenta ha sido configurada exitosamente', 'success', 'low'),
    ((SELECT id FROM tenants LIMIT 1), 'Nuevo Usuario Registrado', 'Se ha registrado un nuevo usuario en el sistema', 'info', 'medium');
