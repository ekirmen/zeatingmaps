-- Script para verificar funciones RPC relacionadas con seat_locks
...
-- 4. Verificar si authenticated puede ejecutar las funciones
SELECT 
    has_function_privilege('authenticated', 'atomic_seat_lock(text, integer, uuid, text)', 'EXECUTE') as can_execute_lock,
    has_function_privilege('authenticated', 'atomic_seat_unlock(text, integer, uuid)', 'EXECUTE') as can_execute_unlock,
    has_function_privilege('authenticated', 'is_seat_available(text, integer)', 'EXECUTE') as can_execute_check;

-- 5. Verificar la definición de las funciones (primeras líneas)
SELECT 
    routine_name,
