-- Configuración completa de métodos de pago con tenant_id
...
-- 2. Insertar métodos de pago si no existen
...
ON CONFLICT (method_id, tenant_id) DO UPDATE SET
    enabled = EXCLUDED.enabled,
    config = EXCLUDED.config,
    updated_at = NOW();

-- 3. Actualizar tenant_id para registros existentes sin tenant_id
UPDATE payment_methods_global 
SET tenant_id = '9dbdb86f-8424-484c-bb76-0d9fa27573c8'::uuid
WHERE tenant_id IS NULL;

-- 4. Verificar resultado
SELECT method_id, enabled, tenant_id, created_at FROM payment_methods_global 
