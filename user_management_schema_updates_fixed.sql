-- 🚀 Actualización del Esquema para Gestión Avanzada de Usuarios (CORREGIDO)
-- Este script agrega los campos necesarios para el sistema de gestión de usuarios mejorado
-- Versión corregida que maneja la estructura actual de la tabla profiles

-- =====================================================
-- VERIFICAR Y ACTUALIZAR TABLA PROFILES
-- =====================================================

-- Agregar campo para nombre si no existe
DO $$
BEGIN
...
-- Agregar campo para email si no existe
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Agregar campos para perfil de usuario y estado
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS perfil VARCHAR(50);
...
-- Agregar campo para permisos detallados (mantener permisos existentes si los hay)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Agregar campo para métodos de pago
...
-- Vista para usuarios activos con sus permisos
...
COMMENT ON COLUMN profiles.perfil IS 'Perfil del usuario (Gerente, Taquilla, etc.)';
