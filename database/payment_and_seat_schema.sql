...
-- Generador de UUID sin depender de extensiones externas
...
AS $$
DECLARE
    random_text text := md5(random()::text || clock_timestamp()::text);
BEGIN
    RETURN (
        substr(random_text, 1, 8) || '-' ||
        substr(random_text, 9, 4) || '-' ||
        substr(random_text, 13, 4) || '-' ||
        substr(random_text, 17, 4) || '-' ||
        substr(random_text, 21, 12)
    )::uuid;
END;
$$;
